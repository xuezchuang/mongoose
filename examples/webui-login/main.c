...
const char* s_listening_url = "http://0.0.0.0:8081";
...
void fn(struct mg_connection* c, int ev, void* ev_data, void* fn_data)
{
	if(ev == MG_EV_HTTP_MSG)
	{
		...
		{
			if(mg_http_match_uri(hm, "/api/init.json"))
			{
				struct mg_http_serve_opts opts = {.root_dir = "layuimini"};
...
int main(void)
{
	...
	while(mgr.conns != NULL) mg_mgr_poll(&mgr, 1000);
	mg_mgr_free(&mgr);
	return 0;
}
